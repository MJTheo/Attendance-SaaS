...
import { api, type CalendarDay, type CalendarStatus, type DayStatusCounts, type StatusDistribution, type UserProfile } from '../lib/api'
import { Header } from '../components/Header'
import { calendarStatusToVariant, type DotVariant } from '../components/StatusDot'
...
function PersonalMonthGrid({ year, month, days }: { year: number; month: number; days: CalendarDay[] }) {
    ...
    <div className="grid grid-cols-7 gap-1.5">
      ...
      {cells.map((date, i) => {
        ...
        return (
          <div
            key={iso}
            className={`flex min-h-[4.5rem] flex-col gap-1 rounded-md border p-1.5 sm:p-2 ${
...
function TeamMonthGrid({ year, month, days }: { year: number; month: number; days: DayStatusCounts[] }) {
